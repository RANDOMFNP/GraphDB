...
void delete_instances_weighted(const string& node_to_delete, const string& input_file) {
    vector<string> lines; 
    string line;
    regex nodetodeletepattern("^Node " + node_to_delete + "\\b");
    regex nodetodeletepattern2("\\bNode " + node_to_delete + "\\[[^\\]]*\\]");
    regex removetrailingcomma(",\\s*\\r?$");
    regex removedoublecomma(",?\\s\\r?$");
